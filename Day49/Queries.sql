...
HAVING COUNT(*) = 3
   AND BOOL_AND(order_count > COALESCE(prev_count, 0));


-- Q223: Employees who have the same salary as the average salary in their job title
SELECT e.*
FROM employees e
JOIN (
...
-- Q225: Employee with the maximum salary in each department
...
-- Q227: Employees whose salary is above company's average but below their department's average
SELECT e.*
FROM employees e
...
-- Q228: Customers who made orders totaling more than the average order amount
...
-- Q229: Total sales amount for each product category, including categories with zero sales
...
-- Q230: Products whose sales have doubled compared to the previous month
...
SELECT product_id, sale_month, total_sales, prev_month_sales
FROM ranked
